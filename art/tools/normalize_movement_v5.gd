extends SceneTree
## One-shot art pipeline for the v5 movement set. Generated frames keep their
## shared source scale; only matte cleanup, canvas placement, and foot alignment
## are applied so compressed and extended poses retain their intended height.
...
const FRAME_PATHS := [
	...
	"res://assets/characters/player/animations/deathcell_hero_run_start_v5_01_anticipation.png",
	"res://assets/characters/player/animations/deathcell_hero_run_start_v5_02_launch.png",
	"res://assets/characters/player/animations/deathcell_hero_run_stop_v5_01_brake.png",
	"res://assets/characters/player/animations/deathcell_hero_run_stop_v5_02_compress.png",
	"res://assets/characters/player/animations/deathcell_hero_run_stop_v5_03_recover.png",
	"res://assets/characters/player/animations/deathcell_hero_run_turn_v5_01_brake.png",
	"res://assets/characters/player/animations/deathcell_hero_run_turn_v5_02_pivot.png",
	"res://assets/characters/player/animations/deathcell_hero_run_turn_v5_03_launch_left.png",
]

const CANVAS_SIZE := Vector2i(2048, 2048)
const BASELINE_Y := 1632
const ALPHA_CUTOFF := 0.72
const MIN_ISLAND_SIZE := 128
...
func _normalize_frame(path: String) -> void:
	...
	var destination := Vector2i(
		(CANVAS_SIZE.x - image.get_width()) / 2,
		BASELINE_Y - used.end.y
	)
	output.blend_rect(image, Rect2i(Vector2i.ZERO, image.get_size()), destination)
	var save_error := output.save_png(absolute_path)
	if save_error != OK:
		push_error("Could not save %s: %s" % [path, save_error])
	else:
		print("normalized %s source_canvas=%s bounds=%s at=%s" % [path, image.get_size(), used, destination])
