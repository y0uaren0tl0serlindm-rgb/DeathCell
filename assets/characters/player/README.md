...
Both idle and run preserve their original eight-frame order, shared 200 x 200 frame canvas, and grounded baseline.
...
2. Produce dedicated roll, run-start, run-stop, turn, jump, fall, hurt, and death poses. Idle, run, and two alternating attack actions are implemented; temporary transitions reuse existing frames so the character identity stays consistent.
